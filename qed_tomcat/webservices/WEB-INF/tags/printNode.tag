									...
									<tr>
										<c:choose>
											<c:when test="${entry.value.$jsonArray}">
												<td><c:choose><c:when test="${entry.value.$doc.isDeprecated}"><s>${entry.key}</s></c:when><c:otherwise>${entry.key}</c:otherwise></c:choose></td>
										...
										<td>
											<c:if test="${typeNode.$defaultValue != null}">default: <b>${typeNode.$defaultValue}</b><br /></c:if>
											<c:if test="${entry.value.$doc != null}"><c:if test="${entry.value.$doc.isDeprecated}"><span style="font-weight:bold;color:#FFE32B;background-color:#7D7D7D;font-size:12px;padding:1px;">DEPRECATED</span></c:if> ${entry.value.$doc.comment}<br />${entry.value.$doc.tags.see}</c:if>
											<c:if test="${fn:length(typeNode.$values) > 0}">
												<ul>
													<c:forEach var="value" items="${typeNode.$values}">
														<li>
															<c:choose>
																...
																<c:when test="${value.$doc.isDeprecated}">
																	<s>${value.$value}</s>
																</c:when>
																<c:otherwise>
																	<i>${value.$value}</i>
															...
															<c:if test="${value.$doc.isDeprecated || value.$doc.comment != null}">:</c:if>
															<c:if test="${value.$doc.isDeprecated}"> <span style="font-weight:bold;color:#FFE32B;background-color:#7D7D7D;font-size:12px;padding:1px;">DEPRECATED</span></c:if>
															<c:if test="${value.$doc.comment != null}"> ${value.$doc.comment}<br />${value.$doc.tags.see}</c:if>
														</li>
													</c:forEach>
												</ul>
											</c:if>
